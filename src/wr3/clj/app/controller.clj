...
(defn index
  ...
  []
  (html
    [:style "body {font-family: 微软雅黑,Verdana,Arial}"]
    [:h1 {:align "center"} "WebFirst/wr3"]
    [:h2 "webapp='" webapp "'"]
    [:h3 "controller的缺省函数。 可由4种url访问到： "]
    [:ul (for [e ["/c" "/c/-" "/c/-/index" "/c/controller/index"] :let [e2 (str webapp e)]] 
           [:li [:a {:href e2} e2]])] 
    [:a {:href "/c/-/apps"} "apps"]))
...
(defn fibs
  "Fib数列和随机数图形"
  []
  (html-head (format "<h2 align=center style='font-family:微软雅黑'>%s</h2>" (:doc (meta (var fibs))))
...
(defn docs
  ...
  (html-body 
    nil
    (join (map #(fdoc %) (ns-publics 'wr3.clj.app.controller)) "<br/><br/>")))
...
(defn- app2 []
  (map #(app1 %) (sort (ns-publics 'wr3.clj.app.controller))))

(defn apps
  "测试多了应用列表"
  []
  (html-apps (concat (app2) ["<h1>cn中文</h1><p>北京汇金科技股份有限公司</p>"])))
